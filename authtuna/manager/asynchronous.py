...
class UserManager:
    ...
    async def create(self, email: str, username: str, password: Optional[str] = None, ip_address: str = 'system',
                     **kwargs) -> User:
        with self._db_manager.get_context_manager_db() as db:
            if await self.get_by_email(email) or await self.get_by_username(username):
            ...
            if password:
                new_user.set_password(password, ip_address, self._db_manager)
            await run_in_threadpool(db.commit)
            await run_in_threadpool(db.refresh, new_user)
        self._db_manager.log_audit_event(new_user.id, "USER_CREATED", ip_address,
                                             {"by": "system" if ip_address == 'system' else 'user_signup'})
        return new_user
    ...
    async def delete(self, user_id: str, ip_address: str = 'system') -> None:
        """Performs a staged deletion by archiving the user's data before removing them."""
        with self._db_manager.get_context_manager_db() as db:
            user = await self.get_by_id(user_id, with_relations=False)
            if not user:
                raise UserNotFoundError("User not found.")
            ...
            db.delete(user)
            await run_in_threadpool(db.commit)
        self._db_manager.log_audit_event(user_id, "USER_DELETED", ip_address, {"archived": True})
    ...
    async def set_password(self, user_id: str, new_password: str, ip_address: str):
        with self._db_manager.get_context_manager_db() as db:
            ...
            if not user:
                raise UserNotFoundError("User not found.")
            user.set_password(new_password, ip_address, self._db_manager)
            await run_in_threadpool(db.commit)
...
class RoleManager:
    ...
    async def get_or_create(self, name: str, defaults: dict = None) -> Tuple[Role, bool]:
        """Gets a role by name, or creates one if it doesn't exist. Idempotent."""
    ...
    async def add_permission_to_role(self, role_name: str, permission_name: str, adder_id: Optional[str] = None):
        """Adds a permission to a role within a single transaction. Idempotent."""
        with self._db_manager.get_context_manager_db() as db:
            ...
            stmt = role_permissions_association.insert().values(
                ...
                added_by_id=adder_id, added_at=time.time()
            )
    ...
    async def assign_to_user(self, user_id: str, role_name: str, assigner_id: str, scope: str = 'global'):
        """Assigns a role to a user within a specific scope. Idempotent."""
        with self._db_manager.get_context_manager_db() as db:
            user = await run_in_threadpool(db.query(User).filter(User.id == user_id).first)
        ...
        self._db_manager.log_audit_event(
            ...
            {"role": role_name, "scope": scope, "by": assigner_id}
        )

    async def get_user_roles_with_scope(self, user_id: str) -> List[dict]:
        """Fetches all roles for a user along with the scope of each assignment."""
        with self._db_manager.get_context_manager_db() as db:
    ...
    async def has_permission(self, user_id: str, permission_name: str, scope_prefix: Optional[str] = None) -> bool:
        """
        FIXED: Checks if a user has a specific permission using an efficient, explicit join
        that avoids cross products and correctly handles scopes.
        """
        with self._db_manager.get_context_manager_db() as db:
            ...
            if scope_prefix:
                stmt = stmt.where(
                    or_(
                        user_roles_association.c.scope == 'global',
...
class PermissionManager:
    ...
    async def get_or_create(self, name: str, defaults: dict = None) -> Tuple[Permission, bool]:
        """Gets a permission by name, or creates one if it doesn't exist. Idempotent."""
...
class SessionManager:
    ...
    async def terminate(self, session_id: str, ip_address: str, errors="ignore"):
        with self._db_manager.get_context_manager_db() as db:
            ...
            if session:
                session.terminate(ip_address, self._db_manager)
                await run_in_threadpool(db.commit)
                return True
            if errors == "ignore":
                return False
    ...
    async def terminate_all_for_user(self, user_id: str, ip_address: str, except_session_id: Optional[str] = None):
        """Terminates all sessions for a user, optionally exempting the current one."""
        with self._db_manager.get_context_manager_db() as db:
...
class TokenManager:
    """Manages one-time tokens for actions like password reset and email verification."""
    ...
    async def validate(self, token_id: str, purpose: str, ip_address: str) -> User:
        """Validates a token. If expired but valid, creates a new one and raises TokenExpiredError."""
        with self._db_manager.get_context_manager_db() as db:
            ...
            if not token_obj.is_valid():
                ...
                token_obj.mark_used(ip_address, self._db_manager)
                await run_in_threadpool(db.commit)
...
class AuthTunaAsync:
    ...
    async def signup(self, username: str, email: str, password: str, ip_address: str) -> Tuple[User, Optional[Token]]:
        """Handles the public user registration workflow."""
    ...
    async def login(self, username_or_email: str, password: str, ip_address: str, region: str, device: str) -> Tuple[
        ...
        with self.db_manager.get_context_manager_db() as db:
            ...
            if not user or not user.check_password(password, ip_address, self.db_manager):
    ...
    async def request_password_reset(self, email: str) -> Optional[Token]:
        """Finds a user by email and creates a password reset token."""
        user = await self.users.get_by_email(email)
        if not user or not settings.EMAIL_ENABLED:
            return None
